...
$(document).ready(function() {
	$('#addButton').click(function(event) {
		...
		$('#tab').append(
			...
			+ "<td style='width: 102px; height: 26px'><select name='outStat.PoolID' id='PoolIDAdd' style='width:100px;height:25px'><option value='MTG_JJC_SC01'>出水池01#</option><option value='MTG_JJC_SC02'>出水池02#</option><option value='MTG_JJC_SC03'  selected='selected'>出水池03#</option></select></td>"
...
var deltr = function(index, event) {
	...
	for(var i = index + 1; i < _len; ++i) {
		...
		$("tr[id='tr" + i + "']").replaceWith("<tr id='tr" + (i-1) + "'>"
				...
				+ "<td style='width: 152px; height: 26px'><input name='outStat.t' id='tAdd' type='date' style='width:150px;height:25px'/></td>"
				+ "<td style='width: 102px; height: 26px'><select name='outStat.PoolID' id='PoolIDAdd' style='width:100px;height:25px'><option value='MTG_JJC_SC01'>出水池01#</option><option value='MTG_JJC_SC02'>出水池02#</option><option value='MTG_JJC_SC03'  selected='selected'>出水03#</option></select></td>"
				+ "<td style='width: 72px; height: 26px'><input style='width:70px;height:25px' id='NTU' name='outStat.NTU' data-options='min:0,precision:3,' /></td>"
				+ "<td style='width: 52px; height: 26px'><input style='width:50px;height:25px' id='Cl' name='outStat.Cl' data-options='min:0,precision:3' /></td>"
				+ "<td style='width: 52px; height: 26px'><input style='width:50px;height:25px' id='Fe' name='outStat.Fe' data-options='min:0,precision:3'/></td>"
				+ "<td style='width: 52px; height: 26px'><input style='width:50px;height:25px' id='Al' name='outStat.Al' data-options='min:0,precision:3'/></td>"
...
//		pageList : [ 3,5,10,15,20], // 可以设置每页记录条数的列表，服务器要加上rows属性
		...
		columns : [[
		            ...
		            {field : 'poolID', title : '出水池编号', align : 'center', sortable : true,width:100,
		            	formatter:function(value){
...
function dealSearch() {
	...
	if(searchMode == 0) {   //如果是从三个radio进行查询
		...
		if(!testTime && !testID) {  //日期空 ID空     searchT=&searchPoolID=
			switch(buttonID) {
				case 'index1':
					titleTemp = '出水池01#  水质分析表';
					ImageTitleTemp = "出水池01#  水质分析图 ";
					newParams = params + 'MTG_JJC_SC01';
					treeURL = '${pageContext.request.contextPath}/searchDataAnalysis.action?dataAnalysis.PoolID=MTG_JJC_SC01';
					break;
				case 'index2':
					titleTemp = '出水池02#  水质分析表';
					ImageTitleTemp = "出水池02#  水质分析图 ";
					newParams = params + 'MTG_JJC_SC02';
					treeURL = '${pageContext.request.contextPath}/searchDataAnalysis.action?dataAnalysis.PoolID=MTG_JJC_SC02';
					break;
				case 'index3':
					titleTemp = '出水池03#  水质分析表';
					ImageTitleTemp = "出水池03#  水质分析图 ";
					newParams = params + 'MTG_JJC_SC03';
					treeURL = '${pageContext.request.contextPath}/searchDataAnalysis.action?dataAnalysis.PoolID=MTG_JJC_SC03';
					break;
				default :
...
//		alert(params);
		...
		switch(buttonID) {  //先查看现在radio的值为多少
			case 'index1':
				title = '出水池01#  水质分析表' + '' + strDate;
				ImageTitle = "出水池01#  水质分析图 " + ' ' + strDate;
				newParams = 'searchT=' + strDate + '&searchPoolID=MTG_JJC_SC01';
				//treeURL = '${pageContext.request.contextPath}/searchDataAnalysis.action?dataAnalysis.PoolID=MTG_JJC_SC01';
				break;
			case 'index2':
				title = '出水池02#  水质分析表' + '' + strDate;
				ImageTitle = "出水池02#  水质分析图 " + ' ' + strDate;
				newParams = 'searchT=' + strDate + '&searchPoolID=MTG_JJC_SC02';
				//treeURL = '${pageContext.request.contextPath}/searchDataAnalysis.action?dataAnalysis.PoolID=MTG_JJC_SC02';
				break;
			case 'index3':
				title = '出水池03#  水质分析表' + '' + strDate;
				ImageTitle = "出水池03#  水质分析图 " + ' ' + strDate;
