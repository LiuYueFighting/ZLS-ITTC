...
//	var params = $("#exportOutStat").serialize();
//	var filename = $('#downloadFilename').val() ;
//	var downloadPath;
//	if(null==filename || ""==filename){
//	downloadPath = "downloadTemp/OutStat.xls";
//	}else{
//	downloadPath = "downloadTemp/"+filename+".xls";
//	}
//	console.log(downloadPath);
//	$.post("exportOutStat.action", params, function(result) {
//	if (result.operateSuccess){
...
function drawImage(){
	...
	{	
		...
		options1.series[i].name='出水浊度<br> '+formPoolID(poollist[i]);
		options1.series[i].type="line";
		options1.series[i].yAxis=0;

		//OutNTU
		options1.series[i+poollist.length] = new Object();
		options1.series[i+poollist.length].data=ClArray[i].sort(); //对ClArray[i]进行排序，否则会造成时间轴上的图错乱
		options1.series[i+poollist.length].name='余氯<br> '+formPoolID(poollist[i]);
		options1.series[i+poollist.length].type="line";
		options1.series[i+poollist.length].yAxis=1;
		
		//PAC
		options2.series[i]= new Object();
		options2.series[i].data=(AlArray[i].sort()); //对FeArray[i]进行排序，否则会造成时间轴上的图错乱
		options2.series[i].name='铝<br> '+formPoolID(poollist[i]);
		options2.series[i].type="line";
		options2.series[i].yAxis=0;

		//FeCl3
		options2.series[i+poollist.length] = new Object();
		options2.series[i+poollist.length].data=FeArray[i].sort(); //对FeArray[i]进行排序，否则会造成时间轴上的图错乱
		options2.series[i+poollist.length].name='铁<br> '+formPoolID(poollist[i]);
