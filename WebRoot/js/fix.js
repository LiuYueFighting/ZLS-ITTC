...
//      document.getElementById("export").onclick = function(){提示};
  ...
  function SchemaFactory(schemaName, mockdata) {
    ...
    this.valveStyleClass = "";

    // this.init = init;

    // 保证了只有第一次调用时才初始化原型方法
    if (typeof this.styleRevelentValves !== "function") {
      // SchemaFactory.prototype.someInterface = someInterface;
      SchemaFactory.prototype.init = init;
      SchemaFactory.prototype.setSchemaInfo = setSchemaInfo;
      SchemaFactory.prototype.setValvesColor = setValvesColor;
    }
  ...
  function init() {
    ...
    // 检查方案是否正确加载
    if(!!this.schemaName) {
      this.setSchemaInfo(this.schemaName);
    } else {
      console.warn("找不到方案名称");
      return;
    }

    try {
      this.valveGroup = this.schemaDict.valveGroup;
    } catch(err) {
      console.log("导入文件中缺少键：valveGroup");
      return;
    }

    if (!this.valveGroup) {
      console.warn("维修/恢复相关阀门不能未空");
      return;
    }

    try {
      this.schemaContext = this.schemaDict.schemaContext;
    } catch(err) {
      console.warn("导入文件中缺少键：schemaContext");
      return;
    }

    if (!this.schemaContext[0]) {
      console.warn("维修/恢复方案不能为空");
      return;
    }

    var valveGroupLength = this.valveGroup.length,
        schemaContextLength = this.schemaContext.length
    that = this;

    switch(this.schemaType) {
      case "fix":
        console.log("fix type");
        this.valveStyleClass="valve-fix-style"
        break;
      case "restore":
        console.log("restore type");
        this.valveStyleClass="valve-restore-style"
        break;
      default:
        console.warn("没有对应的维修方案类型，确保以'fix_'或'restore_'形式命名维修方案");
        return;
    }
  ...
  function setSchemaInfo(schemaName) {
    schemaNameInfo = schemaName.split(/[\,_-]+/);
    this.schemaType = schemaNameInfo[0];
    this.schemaTarget = schemaNameInfo[1];
  }

  function setSechmaGenerator() {
    console.log(that.schemaDict.schemaDownload);
    document.getElementById("export").href = that.schemaDict.schemaDownload;
    $("#export").text("生成方案");
    $("#reset").text("重置");
  }

  function setPanelInfo() {
    console.log("Fill in :" + that.schemaDict.schemaName);
    // Pay attention to avoid 'that.schemaDict[schemaName] because of name conflict'
    $("#fix_head").html(that.schemaDict.schemaName);

    var schemaContext = that.schemaContext,
    ...
    for (; i < length; i++) {
      ...
      // console.group('panel info');
      // console.log('offset: ' + offset);
      // console.log('content: ', schemaContext[i].content);
      // console.log('result: ', schemaContext[i].result);
      // console.groupEnd();
    ...
    console.log(that.schemaDict.status);
    $("#init_stat").html(that.schemaDict.status);
  }

  function setValvesColor() {
    var ids 		= that.valveGroup,
        length  = that.valveGroup.length,
        type    = that.schemaType,
        i 			= 0;
  ...
  function setValveColor(id, type) {
    ...
    image.src = valveSrc;
    console.log(id);
    console.log(valveTagId);
    console.log(valveTagIdSelector);
    $(valveTagIdSelector).addClass("FMStyle");

    buildingTagId = "#name_" + that.schemaTarget.toUpperCase();
    console.log(buildingTagId);
...
unittest();
  ...
  function unittest() {
    var schema = new SchemaFactory("fix_hx_right", mockdata);
